    ...
    const fetchPosts = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:7180/community/post?page=${page}&limit=5`
        );
    ...
    <>
      <div className="min-h-screen w-full bg-slate-50">
        <header className="relative h-48 w-full bg-[url('/vectorImg/community.jpg')] bg-cover bg-center md:h-56">
          ...
          <div className="relative z-10 flex h-full items-center justify-center">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-4xl font-bold text-slate-800 md:text-5xl">Community</h1>
            </div>
          </div>
        </header>
        ...
        <div className="container mx-auto max-w-4xl px-4 pb-16">
            ...
            <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
              <div>
                ...
                <div className="mt-2 flex items-center gap-2 text-sm text-slate-500">
                  ...
                  <span>Public Group • {totalPosts} posts</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1 overflow-x-auto">
                {TABS.map((tab) => (
                  <button key={tab} onClick={() => setActiveTab(tab)}
                    className={`whitespace-nowrap rounded-md px-3 py-1.5 text-sm font-semibold transition-colors ${activeTab === tab ? "bg-green-100 text-green-700" : "text-slate-600 hover:bg-slate-100"}`}>
                    {tab}
                  </button>
                ))}
              </div>
              <div className="flex items-center gap-2">
                <button aria-label="Search" className="rounded-full p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-800">
                  <MagnifyingGlassIcon className="h-5 w-5" />
                </button>
                <button aria-label="More options" className="rounded-full p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-800">
          ...
          <main className="mt-8">
            {isLoading && page === 1 ? (
              <p className="py-12 text-center text-slate-500">Loading feed...</p>
            ) : error ? (
              <p className="py-12 text-center text-red-500">{error}</p>
            ) : activeTab === "Discussion" ? (
              <Discussion posts={posts} user={user} onShare={handleShare} />
            ) : activeTab === "Media" ? (
              <Media />
            ) : (
              <p className="py-12 text-center text-slate-500">This section is coming soon.</p>
            )}
      ...
      {user && (
        <Link to={`/addproduct`} aria-label="Create a new post"
