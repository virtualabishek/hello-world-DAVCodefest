        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="min-h-[180px] flex flex-col justify-between">
            <div>
              ...
              <p className="mt-4 text-slate-300">{t('footer.tagline')}</p>
            </div>
            <div className="mt-6 flex gap-4">
              {socialLinks.map(item => (
                <a key={item.name} href={item.href} className="text-slate-400 hover:text-green-500" aria-label={item.name}>
          ...
          <div className="min-h-[180px] flex flex-col justify-between">
            <div>
              ...
              <ul className="mt-4 space-y-2">
                ...
                <li><Link to="/news" className="text-slate-300 hover:text-green-500">{t('navigation.news')}</Link></li>
              </ul>
            </div>
                 </div>
          {/* Legal & Language */}
          <div className="min-h-[180px] flex flex-col justify-between">
            <div>
              ...
              <ul className="mt-4 space-y-2">
                <li><a href="#" className="text-slate-300 hover:text-green-500">{t('footer.privacy')}</a></li>
                <li><a href="#" className="text-slate-300 hover:text-green-500">{t('footer.terms')}</a></li>
              </ul>
            </div>
            <div className="mt-8">
              <button
                onClick={() => i18n.changeLanguage(i18n.language === 'np' ? 'en' : 'np')}
                className="rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-green-700 transition"
              >
                {i18n.language === 'np' ? 'Switch to English' : 'नेपालीमा हेर्नुहोस्'}
              </button>
            </div>
          </div>
        </div>
        <div className="mt-12 border-t border-slate-700 pt-8 text-center text-sm text-slate-400">
          <p>© {new Date().getFullYear()} SaralKrishi. {t('footer.rights')}</p>
        </div>
      </div>
    </footer>
  );
