  ...
  if (!name || !email || !password) {
    return res.status(400).json({ message: 'Username, email, and password are required' });
      ...
      console.log("ip",ip)
        ...
        const locationResponse = await axios.get(`https://ipinfo.io/${ip}/json?token=${process.env.LOCATIONAPIKEY}`);
    ...
    const newUser = new User({
      ...
      verificationToken,
        verificationTokenExpiresAt: Date.now() + 24 * 60 * 60 * 1000,
            ...
            const result = await sendNotification({ userId:newUser._id, topic:"Sign up completed",message: "Welcome to the KhetAI",image:"https://png.pngtree.com/png-clipart/20190516/original/pngtree-check-mark-icon-design-template-vector-isolated-png-image_4085369.jpg" });
      console.log(newUser);
      console.log(result)
...
try {
    ...
    await user.save()
    const result = await sendNotification({ userId:user._id, topic:"Verification Completed",message: "Your Id has been Verified Successfully",image:"https://png.pngtree.com/png-clipart/20190516/original/pngtree-check-mark-icon-design-template-vector-isolated-png-image_4085369.jpg" });
...
console.log(req.body)
   ...
   const loguser= await user.save()
        ...
        const result = await sendNotification({ userId:loguser._id, topic:"Login Completed",message: `Your id has been Loggedin at ${now.toString()}`,image:"https://png.pngtree.com/png-clipart/20190516/original/pngtree-check-mark-icon-design-template-vector-isolated-png-image_4085369.jpg" });
    console.log("result",result)
    return res.status(200).json({success:true,message:"login successfull",token:token,user:{
        ...user._doc,password:undefined
    }})
    ...
    console.log(error)
    return res.status(400).json({success:false,message:"login unsuccessfull"})
...
export const Home = async (req, res) => {
    try {
        ...
        const locationResponse = await axios.get(`https://ipinfo.io/${userIP}/json?token=${process.env.LOCATIONAPIKEY}`);
        ...
        const weatherResponse = await axios.get(`https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=metric&appid=${process.env.WEATHERAPIKEY}`);
...
export const weatherAPI = async (req, res) => {
    try {
        ...
        const response = await axios.get(`https://api.openweathermap.org/data/2.5/forecast?q=Birgunj&units=metric&appid=${WEATHERAPI}`);
        
        // Check if API call was successful
        if (response.status !== 200) {
            return res.status(response.status).json({ success: false, message: "Failed to fetch weather data" });
        }
...
if (userId === req.params.friendId) {
  return res.status(200).json({ message: "You cannot send a friend request to yourself" });
}
...
const Friend=await User.findById(req.params.friendId)    
 const usermessage = await sendNotification({ userId:userId, topic:` Friend Request Sent`,message: `You Send a Friend Request to ${Friend.username}`,image:"https://png.pngtree.com/png-clipart/20190516/original/pngtree-check-mark-icon-design-template-vector-isolated-png-image_4085369.jpg" });
    console.log("usermessage", usermessage)
    console.log("----------")
 const friendmessage = await sendNotification({ userId:Friend._id, topic:` Friend Request received`,message: `You Got a Friend Request From ${user.username}`,image:"https://png.pngtree.com/png-clipart/20190516/original/pngtree-check-mark-icon-design-template-vector-isolated-png-image_4085369.jpg" });
...
try {
    ...
    if (!user) { 
        ...
        console.log(user)
    return res.status(200).json({ message: "Friend fetched successfully" ,friends:user.friends});
} catch (error) {
